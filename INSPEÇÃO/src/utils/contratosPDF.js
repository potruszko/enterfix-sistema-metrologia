...
function adicionarCabecalho(doc, numeroContrato, statusContrato) {
  ...
  doc.text(`Contrato: ${numeroContrato}`, ESTILOS.larguraPagina - ESTILOS.margemDireita, 15, { align: 'right' });
...
function adicionarRodape(doc, numeroPagina, totalPaginas) {
  ...
  doc.text(textoRodape, ESTILOS.larguraPagina / 2, y, { align: 'center' });
  
  const contatoRodape = `Tel: ${DADOS_ENTERFIX.telefone} - Email: ${DADOS_ENTERFIX.email} - ${DADOS_ENTERFIX.website}`;
  doc.text(contatoRodape, ESTILOS.larguraPagina / 2, y + 4, { align: 'center' });

  // Número da página
  doc.setFontSize(ESTILOS.tamanhoRodape);
  doc.text(`Página ${numeroPagina} de ${totalPaginas}`, ESTILOS.larguraPagina - ESTILOS.margemDireita, y, { align: 'right' });
}
...
export async function gerarPDFContrato(dadosContrato) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.text(titulo, ESTILOS.larguraPagina / 2, y, { align: 'center' });
      
      y += 8;
      doc.setFontSize(14);
      doc.text(`Nº ${dadosContrato.numero_contrato}`, ESTILOS.larguraPagina / 2, y, { align: 'center' });
      ...
      y = adicionarParagrafo(doc, preambulo, y, { estilo: 'italic' });
      y += 3;

      // CONTRATADA
      y = adicionarParagrafo(doc, 'CONTRATADA:', y, { estilo: 'bold' });
      ...
      // CONTRATANTE
      y = adicionarParagrafo(doc, 'CONTRATANTE:', y, { estilo: 'bold' });
      ...
      y = adicionarParagrafo(doc, introducao, y, { estilo: 'italic' });
      y += 8;
      ...
      if (CLAUSULAS_ESPECIFICAS[dadosContrato.tipo_contrato]) {
        ...
        y = adicionarTituloSecao(doc, 'CLÁUSULA ESPECIAL - DISPOSIÇÕES ESPECÍFICAS', y);
      ...
      y = adicionarParagrafo(doc, encerramento, y, { alinhamento: 'center', estilo: 'italic' });
      y += 10;

      const dataAssinatura = `${DADOS_ENTERFIX.cidade}/${DADOS_ENTERFIX.estado}, ${dataExtenso(new Date().toISOString().split('T')[0])}.`;
      y = adicionarParagrafo(doc, dataAssinatura, y, { alinhamento: 'center' });
      y += 20;
      ...
      y += 5;
      y = adicionarParagrafo(doc, DADOS_ENTERFIX.razaoSocial, y, { alinhamento: 'center', estilo: 'bold' });
      y = adicionarParagrafo(doc, `CNPJ: ${DADOS_ENTERFIX.cnpj}`, y, { alinhamento: 'center', tamanhoFonte: 9 });
      y = adicionarParagrafo(doc, 'CONTRATADA', y, { alinhamento: 'center', estilo: 'bold' });
      y += 20;

      // Bloco CONTRATANTE
      doc.line(centroX - larguraAssinatura / 2, y, centroX + larguraAssinatura / 2, y);
      y += 5;
      y = adicionarParagrafo(doc, cliente.razao_social, y, { alinhamento: 'center', estilo: 'bold' });
      y = adicionarParagrafo(doc, cliente.tipo_pessoa === 'juridica' ? `CNPJ: ${cliente.cnpj}` : `CPF: ${cliente.cpf}`, y, { 
        alinhamento: 'center', 
        tamanhoFonte: 9 
      });
      y = adicionarParagrafo(doc, 'CONTRATANTE', y, { alinhamento: 'center', estilo: 'bold' });
      y += 20;

      // TESTEMUNHAS
      y = adicionarParagrafo(doc, 'TESTEMUNHAS:', y, { estilo: 'bold' });
      y += 10;
...
export async function gerarEUploadPDFContrato(supabase, dadosContrato) {
  try {
    // 1. Gerar PDF
    const { blob, filename } = await gerarPDFContrato(dadosContrato);
    
    // 2. Upload para Supabase
    const { path, publicUrl } = await uploadPDFContrato(
      supabase,
      blob,
      filename,
      dadosContrato.id
