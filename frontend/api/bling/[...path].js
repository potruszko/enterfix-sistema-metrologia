    ...
    try {
        ...
        if (s0 === 'auth') {
            if (s1 === 'url' && method === 'GET') {
                ...
                if (!configured) return res.json({
                    url: '',
                    configurado: false,
                    detalhe: 'Configure BLING_CLIENT_ID e BLING_CLIENT_SECRET'
                });
        ...
        if (s0 === 'preview' && s1 && method === 'GET') {
            ...
            if (produtoBling) {
                ...
                const cb = parseFloat(produtoBling.precoCusto || 0),
                    cl = parseFloat(produto.custo_total || 0);
                if (Math.abs(cb - cl) > 0.01) mudancas.push({
                    campo: 'custo',
                    local: cl,
                    bling: cb
                });
        ...
        return res.status(404).json({
            erro: 'Rota Bling não encontrada'
        });
