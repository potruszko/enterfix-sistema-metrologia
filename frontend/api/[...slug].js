...
async function readBody(req) {
  ...
  if (ct.includes('application/json') && raw) {
    try { return { json: JSON.parse(raw), raw }; } catch { return { json: {}, raw }; }
  }
  return { json: {}, raw };
}
...
export default async function handler(req, res) {
  ...
  const { json: body, raw } = await readBody(req);
  req.body = body;
  try {
...
async function gerarNumeroOP() {
  ...
  let seq = 1;
  if (last) { const m = last.numero.match(/(\d+)$/); if (m) seq = parseInt(m[1]) + 1; }
...
async function route(req, res, slug, rawBody) {
  ...
  if (s0 === 'blanks') {
    if (!s1) {
      if (method === 'GET') {
        const { rosca, diametro_furo } = req.query;
        let sql = 'SELECT * FROM blanks WHERE 1=1';
        const params = [];
        if (rosca) { params.push(rosca); sql += ` AND rosca = $${params.length}`; }
        if (diametro_furo) { params.push(parseFloat(diametro_furo)); sql += ` AND diametro_furo = $${params.length}`; }
        sql += ' ORDER BY rosca, diametro_corpo';
        return res.json(await query(sql, params));
      }
      if (method === 'POST') {
        const { codigo, descricao, rosca, diametro_corpo, diametro_furo, comprimento, material, custo } = req.body;
        if (!codigo || !rosca) return res.status(400).json({ erro: 'codigo e rosca são obrigatórios' });
        try {
          ...
          return res.status(201).json({ id: row.id, mensagem: 'Blank criado' });
        } catch (err) {
      ...
      if (method === 'PUT') {
        ...
        return res.json({ id: parseInt(id), mensagem: 'Blank atualizado' });
      }
      if (method === 'DELETE') {
        ...
        return res.json({ mensagem: 'Blank removido' });
      }
    }
  }
  ...
  if (s0 === 'hastes') {
    if (!s1) {
      ...
      if (method === 'POST') {
        const { codigo, descricao, rosca, comprimento, diametro, material, custo } = req.body;
        if (!codigo || !rosca) return res.status(400).json({ erro: 'codigo e rosca são obrigatórios' });
        try {
          const row = await queryOne(
          ...
          return res.status(201).json({ id: row.id, mensagem: 'Haste criada' });
        } catch (err) {
          if (err.code === '23505') return res.status(409).json({ erro: `Código ${codigo} já existe` });
          throw err;
        }
      ...
      if (method === 'PUT') {
        ...
        return res.json({ id: parseInt(id), mensagem: 'Haste atualizada' });
      }
      if (method === 'DELETE') {
        ...
        return res.json({ mensagem: 'Haste removida' });
      }
    }
  }
  ...
  if (s0 === 'esferas') {
    if (!s1) {
      ...
      if (method === 'POST') {
        const { codigo, descricao, material, diametro, tem_furo, custo } = req.body;
        if (!codigo || !material) return res.status(400).json({ erro: 'codigo e material são obrigatórios' });
        try {
          const row = await queryOne(
          ...
          return res.status(201).json({ id: row.id, mensagem: 'Esfera criada' });
        } catch (err) {
          if (err.code === '23505') return res.status(409).json({ erro: `Código ${codigo} já existe` });
          throw err;
        }
      ...
      if (method === 'PUT') {
        ...
        return res.json({ id: parseInt(id), mensagem: 'Esfera atualizada' });
      }
      if (method === 'DELETE') {
        ...
        return res.json({ mensagem: 'Esfera removida' });
      }
    }
  }
  ...
  if (s0 === 'mao-de-obra') {
    if (!s1) {
      ...
      if (method === 'POST') {
        const { codigo, descricao, tipo, custo_hora, custo_por_unidade } = req.body;
        if (!codigo || !descricao) return res.status(400).json({ erro: 'codigo e descricao são obrigatórios' });
        try {
          const row = await queryOne(
          ...
          return res.status(201).json({ id: row.id, mensagem: 'Mão de obra criada' });
        } catch (err) {
          if (err.code === '23505') return res.status(409).json({ erro: `Código ${codigo} já existe` });
          throw err;
        }
      ...
      if (method === 'PUT') {
        ...
        return res.json({ id: parseInt(id), mensagem: 'Mão de obra atualizada' });
      }
      if (method === 'DELETE') {
        ...
        return res.json({ mensagem: 'Mão de obra removida' });
      }
    }
  }
  ...
  if (s0 === 'produtos') {
    if (!s1) {
      ...
      if (method === 'POST') {
        ...
        if (!codigo || !nome || !tipo) return res.status(400).json({ erro: 'Campos obrigatórios: codigo, nome, tipo' });
      ...
      if (method === 'PUT') {
        const { codigo, nome, tipo, rosca, comprimento_total, diametro_esfera, preco_venda, margem, status, observacoes, componentes = [] } = req.body;
        const client = await getPool2().connect();
        try {
          await client.query('BEGIN');
          await client.query(
          ...
          await client.query('COMMIT');
          return res.json({ id: parseInt(id), mensagem: 'Produto atualizado' });
        } catch (err) {
          await client.query('ROLLBACK');
          throw err;
        } finally {
  ...
  if (s0 === 'composicoes') {
    if (s1 === 'sugerir-blank' && method === 'POST') {
      const { rosca, diametro_furo } = req.body;
      if (!rosca) return res.status(400).json({ erro: 'rosca é obrigatória' });
    ...
    if (s1 === 'calcular-haste' && method === 'POST') {
      ...
      if (!comprimento_total || !blank_id) return res.status(400).json({ erro: 'comprimento_total e blank_id são obrigatórios' });
      ...
      return res.json({ comprimento_haste: Math.max(0, comprimento_haste), raio_esfera: raio });
    }
    if (s1 === 'calcular-custo' && method === 'POST') {
      const { componentes = [], margem = 0 } = req.body;
      ...
      return res.json({ custo_total, preco_sugerido, margem_calculada: parseFloat(margem) });
    }
    if (s1 && !s2) {
      ...
      if (method === 'GET') {
        ...
        return res.json({ ...produto, componentes, custo_total, margem_calculada });
      }
    }
  }
  ...
  // ═══════════════════════════════════════════════════════ IMPORTAR ═════════
  if (s0 === 'importar') {
    if (!rawBody) return res.status(400).json({ erro: 'Envie o CSV no body da requisição' });
    ...
    if (s1 === 'blanks' && method === 'POST') {
      ...
      for (const row of rows) {
        ...
        } catch (err) { resultados.erros.push(`${codigo}: ${err.message}`); }
      }
      return res.json({ mensagem: `Importação concluída: ${resultados.importados} blanks processados`, total_csv: rows.length, ...resultados });
    }
    ...
    if (s1 === 'esferas' && method === 'POST') {
      ...
      return res.json({ mensagem: `Importação concluída: ${resultados.importados} esferas processadas`, total_csv: rows.length, ...resultados });
    }
  }
  ...
  if (s0 === 'centros-trabalho') {
    if (!s1) {
      ...
      const id = s1;
      if (method === 'PUT') {
        const { nome, custo_hora_maquina, custo_hora_operador, status, observacoes } = req.body;
        await query(
  ...
  if (s0 === 'ordens-producao') {
    if (!s1) {
      ...
      return res.json({ mensagem: 'OP iniciada' });
      ...
      return res.json({ mensagem: 'OP concluída com sucesso', custo_real, custo_processo, custo_material: parseFloat(op.custo_material) || 0 });
    }
  }
  ...
  if (s0 === 'apontamentos') {
    if (!s1 && method === 'POST') {
      ...
      return res.status(201).json({ id: row.id, mensagem: 'Apontamento iniciado' });
    }
    ...
    if (s1 && s2 === 'concluir' && method === 'PUT') {
      ...
      return res.json({ mensagem: 'Apontamento concluído', duracao_min, custo_calculado });
    }
    if (s1 && s2 === 'pausar' && method === 'PUT') {
      ...
      return res.json({ mensagem: 'Apontamento pausado', duracao_min: parseFloat(durMin.min) });
    }
  }
  ...
  if (s0 === 'bling') {
    ...
    if (s1 === 'auth') {
      if (s2 === 'url' && method === 'GET') {
        ...
        if (!configured) return res.json({ url: '', configurado: false, detalhe: 'Configure BLING_CLIENT_ID e BLING_CLIENT_SECRET nas variáveis de ambiente' });
      ...
      if (s2 === 'callback' && method === 'GET') {
        ...
        try {
          ...
          return res.status(500).json({ erro: 'Falha na autenticação Bling', detalhe: err.response?.data || err.message });
        }
      }
      ...
      if (s2 === 'status' && method === 'GET') {
        ...
        return res.json({ configurado: !!tokens.bling_access_token, valido: expiry ? expiry > new Date() : false, expira_em: expiry ? expiry.toISOString() : null });
      }
    }
    ...
    if (s1 === 'sincronizar' && s2 && method === 'POST') {
      ...
      let blingId = produto.bling_id; let respData;
      if (blingId) {
        const resp = await client.patch(`/produtos/${blingId}`, payload); respData = resp.data;
      } else {
        const resp = await client.post('/produtos', payload); respData = resp.data;
      ...
      return res.json({ mensagem: 'Produto sincronizado com Bling', bling_id: blingId, resposta: respData });
    }
